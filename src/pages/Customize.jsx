    ...
    <div className="min-h-screen pt-32 bg-white">
      ...
      <div className="max-w-7xl mx-auto px-6 mb-12">
        <button 
          ...
          className="flex items-center gap-2 text-xs font-bold text-slate-400 hover:text-soul-blue transition-colors mb-8 uppercase tracking-widest"
        >
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-100 pb-10">
          <div>
            <span className="text-xs font-black text-soul-blue mb-3 block uppercase tracking-[0.2em]">
          ...
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              <div style={{ backgroundColor: '#2B4A8C' }} className={`w-10 h-10 rounded-xl flex items-center justify-center font-black ${step >= 1 ? 'text-white shadow-lg shadow-blue-200' : 'bg-slate-100 text-slate-400'}`}>1</div>
              <span className={`text-sm font-bold ${step === 1 ? 'text-slate-800' : 'text-slate-400'}`}>Style</span>
            </div>
            <div className="h-[2px] w-8 bg-slate-100"></div>
            <div className="flex items-center gap-2">
              <div style={step >= 2 ? { backgroundColor: '#2B4A8C' } : {}} className={`w-10 h-10 rounded-xl flex items-center justify-center font-black ${step >= 2 ? 'text-white shadow-lg shadow-blue-200' : 'bg-slate-100 text-slate-400'}`}>2</div>
        ...
        {step === 1 && (
          ...
          >
            <div className="max-w-7xl mx-auto text-center">
              <h2 className="text-4xl font-black mb-16 text-slate-800">Who's <span className="text-soul-blue text-5xl">Traveling?</span></h2>
                ...
                {experiences.map((exp) => (
                  ...
                  >
                    ...
                    <div className="absolute inset-0 p-10 flex flex-col items-center justify-end text-white">
                      <div className="w-16 h-16 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center mb-6 group-hover:bg-soul-blue transition-all duration-500">
                        {exp.id === 'solo' && <User className="w-8 h-8" />}
                        {exp.id === 'adventure' && <Compass className="w-8 h-8" />}
                        {exp.id === 'couple' && <Heart className="w-8 h-8" />}
                      </div>
                      <span className="text-xs font-black text-soul-blue mb-2 opacity-0 group-hover:opacity-100 transition-all transform translate-y-4 group-hover:translate-y-0 uppercase tracking-widest">
                        {exp.tagline}
                      </span>
          ...
          >
            <div className="max-w-7xl mx-auto">
              <div className="text-center mb-16">
                <h2 className="text-4xl font-black mb-4 text-slate-800">Select your <span className="text-soul-blue">Tier</span></h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-6xl mx-auto">
                {pricingTiers.map((tier) => (
                  <div key={tier.id} className={`p-12 border border-slate-100 rounded-[32px] flex flex-col transition-all duration-500 hover:shadow-2xl relative overflow-hidden ${tier.id === 'medium' ? 'bg-slate-900 text-white shadow-2xl scale-105 z-10' : 'bg-white'}`}>
                    {tier.id === 'medium' && (
                      <div style={{ backgroundColor: '#2B4A8C' }} className="absolute top-0 right-0 text-white text-[10px] uppercase font-black px-5 py-2.5 rounded-bl-2xl">
                        Best Seller
                      </div>
                    )}
                    
                    <h3 className="text-3xl font-black mb-6 text-soul-blue">{tier.name}</h3>
                    <div className="text-5xl font-black text-soul-blue mb-10">{tier.price} <span className="text-xs font-bold text-slate-400 block mt-2">/ PERSON</span></div>
                    ...
                    <button 
                      ...
                      style={tier.id === 'medium' ? { backgroundColor: '#2B4A8C' } : {}}
                      className={`w-full py-5 rounded-2xl font-black text-lg transition-all ${tier.id === 'medium' ? 'text-white shadow-xl shadow-blue-900/20' : 'bg-slate-50 text-slate-800 hover:bg-slate-100'}`}
                    >
                      View Itinerary
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 h-full">
                  ...
                  <div className="lg:col-span-2 relative min-h-[400px]">
                    ...
                    <div className="absolute bottom-12 left-12 right-12 text-white">
                      <span className="text-xs font-black text-soul-blue mb-4 block uppercase tracking-widest">Selected Journey</span>
                      <h2 className="text-5xl font-black mb-8 leading-tight">{country.name} <br /><span className="text-soul-blue">{selectedPackage?.name}</span></h2>
                      <div className="flex items-center gap-10">
                        <div className="flex items-center gap-3">
                          <Clock className="w-5 h-5 text-soul-blue" />
                          <span className="text-sm font-bold">5 Days / 4 Nights</span>
                        </div>
                        <div className="flex items-center gap-3">
                          <User className="w-5 h-5 text-soul-blue" />
                  ...
                  <div className="lg:col-span-3 p-12 md:p-20 bg-white">
                    <div className="space-y-12">
                      <div className="flex items-center justify-between border-b border-slate-100 pb-10">
                        <div>
                          <h3 className="text-3xl font-black text-soul-blue">Trip Itinerary</h3>
                        ...
                        <div className="text-right">
                          ...
                          <span className="text-4xl font-black text-soul-blue">{selectedPackage?.price}</span>
                        </div>
                      </div>
                        ...
                        {itineraries.default.map((day, i) => (
                          <div key={i} className="flex gap-8 group">
                            <div className="flex flex-col items-center">
                              <div className="w-10 h-10 bg-blue-50 text-soul-blue rounded-xl flex items-center justify-center font-black group-hover:bg-soul-blue group-hover:text-white transition-all">
                                {day.day}
                              </div>
                              {i !== itineraries.default.length - 1 && <div className="w-0.5 h-full bg-slate-50 mt-4"></div>}
                            </div>
                            <div>
                              <h4 className="text-xl font-bold mb-3 text-soul-blue">{day.title}</h4>
                      ...
                      <div className="pt-10 flex gap-6">
                        <button 
                          ...
                          className="flex-grow text-white py-6 rounded-[20px] font-black text-xl shadow-xl shadow-blue-200 hover:opacity-90 transition-all active:scale-95"
                        >
                          Confirm & Book Now
                        </button>
                        <button 
